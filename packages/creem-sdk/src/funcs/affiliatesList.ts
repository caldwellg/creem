...
export function affiliatesList(
  ...
  PageIterator<
    Result<
      operations.ListAffiliatesResponse,
      | CreemError
...
> {
  const input: operations.ListAffiliatesRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) => operations.ListAffiliatesRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/v1/affiliates")();
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListAffiliatesResponse$inboundSchema, {
      key: "Result",
    }),
